...
{
    public class QueryCosmosStatisticsActivity
    {
        [FunctionName(nameof(QueryCosmosStatisticsActivity))]
        public async Task<dynamic> Run([ActivityTrigger] IDurableActivityContext context, ILogger log)
        {
            ...
            var docTmp = await Utils.MeasureQueryV3<StatisticsDocument>(CosmosContainer.Container, statsQuery);
